...
async function saveVisibleState(){
 ...
 try{
  ...
  const snap=await getDoc(root),x=snap.exists()?snap.data():{},schedule={...(x.schedule||{})},baseline={};
  for(const k of Object.keys(schedule))if(k.startsWith(month+'-'))delete schedule[k];
  document.querySelectorAll('#schedule select[data-k]').forEach(s=>{const k=s.dataset.k;if(k?.startsWith(month+'-')&&s.value){schedule[k]=s.value;baseline[k]=s.value}});
  const savedStates={...(x.savedStates||{}),[month]:baseline};
  await setDoc(root,{schedule,savedStates,updatedAt:new Date().toISOString()},{merge:true});
...
function install(){const gen=document.getElementById('generate');if(!gen)return;let b=document.getElementById('saveStateBtn');if(!b){b=document.createElement('button');b.id='saveStateBtn';b.type='button';b.textContent='SALVA STATO';b.className='adminOnly';gen.insertAdjacentElement('afterend',b)}if(b.dataset.saveStateBound!=='1'){b.dataset.saveStateBound='1';b.addEventListener('click',saveVisibleState)}}
document.getElementById('month')?.addEventListener('change',e=>{const m=e.target.value;if(!m)return;localStorage.setItem('turniLastMonth',m);setTimeout(()=>location.reload(),80)},true);
